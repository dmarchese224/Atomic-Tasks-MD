...
function buildTaskLine(task){ const tags = task.tags.map(t=>`#${t.replace(/^#/,'')}`).join(' '); const due = task.due ? ` due:${task.due}` : ''; const time = task.time ? ` time:${task.time}` : ''; return `- [${task.done?'x':' '}] ${task.text}${due}${time}${tags ? ' ' + tags : ''}`.trim(); }
function parseTaskLine(line, index){ const m = line.match(/^\s*- \[( |x|X)\]\s+(.*)$/); if(!m) return null; const done = m[1].toLowerCase()==='x'; const body = m[2]; const dueMatch = body.match(/(?:^|\s)due:(\d{4}-\d{2}-\d{2})(?=\s|$)/); const timeMatch = body.match(/(?:^|\s)time:(\d{2}:\d{2})(?=\s|$)/); const due = dueMatch ? dueMatch[1] : ''; const time = timeMatch ? timeMatch[1] : ''; const tags = [...body.matchAll(/(^|\s)#([a-zA-Z0-9_-]+)/g)].map(x=>x[2]); let text = body.replace(/(?:^|\s)due:\d{4}-\d{2}-\d{2}(?=\s|$)/g,'').replace(/(?:^|\s)time:\d{2}:\d{2}(?=\s|$)/g,'').replace(/(^|\s)#[a-zA-Z0-9_-]+/g,'').replace(/\s+/g,' ').trim(); return { id:`task-${index}`, done, text, due, time, tags, raw:line }; }
...
function updateTaskPreview(){
  const draft = { done:false, text:$('taskTextInput').value.trim(), due:$('taskDueInput').value, time:$('taskTimeInput').value, tags:normalizeTags($('taskTagsInput').value) };
...
async function saveTask(){
  ...
  if(state.editingTaskId){
    ...
    task.text = text; task.due = due; task.time = time; task.tags = tags;
  } else {
    state.tasks.unshift({ id:`task-${Date.now()}`, done:false, text, due, time, tags });
  }
  await saveTaskFile();
  renderAll();
  closeEditor();
...
function loadDemoTasks(){
  state.tasks = [
    { id:'demo-1', done:false, text:'Finish dissertation chapter outline', due:todayStr(), time:'09:00', tags:['school'] },
    { id:'demo-2', done:false, text:'Review fatigue article highlights', due:'2026-04-10', time:'14:30', tags:['research','school'] },
    { id:'demo-3', done:false, text:'Schedule dentist appointment', due:'2026-04-08', time:'11:15', tags:['personal'] },
    { id:'demo-4', done:true, text:'Update Atomic Notes MD app', due:'2026-04-05', time:'18:00', tags:['projects'] }
  ];
  renderAll();
}
