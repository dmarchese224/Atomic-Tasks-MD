...
function buildTaskLine(task){ const tags = task.tags.map(t=>`#${t.replace(/^#/,'')}`).join(' '); const due = task.due ? ` due:${task.due}` : ''; return `- [${task.done?'x':' '}] ${task.text}${due}${tags ? ' ' + tags : ''}`.trim(); }
function parseTaskLine(line, index){ const m = line.match(/^\s*- \[( |x|X)\]\s+(.*)$/); if(!m) return null; const done = m[1].toLowerCase()==='x'; const body = m[2]; const dueMatch = body.match(/(?:^|\s)due:(\d{4}-\d{2}-\d{2})(?=\s|$)/); const due = dueMatch ? dueMatch[1] : ''; const tags = [...body.matchAll(/(^|\s)#([a-zA-Z0-9_-]+)/g)].map(x=>x[2]); let text = body.replace(/(?:^|\s)due:\d{4}-\d{2}-\d{2}(?=\s|$)/g,'').replace(/(^|\s)#[a-zA-Z0-9_-]+/g,'').replace(/\s+/g,' ').trim(); return { id:`task-${index}`, done, text, due, tags, raw:line }; }
...
function filteredTasks(){
  ...
  if(state.searchQuery.trim()){
    ...
    tasks = tasks.filter(t => t.text.toLowerCase().includes(q) || t.tags.some(tag=>tag.toLowerCase().includes(q)) || (t.due||'').includes(q));
  }
  ...
  tasks.forEach(task=>{
    ...
    row.innerHTML = `
      ...
      <div class="task-main">
        ...
        <div class="task-meta">
          ${task.due ? `<span class="task-chip task-due ${dueClass(task.due)}">${esc(task.due)}</span>` : ''}
...
function updateTaskPreview(){
  const draft = { done:false, text:$('taskTextInput').value.trim(), due:$('taskDueInput').value, tags:normalizeTags($('taskTagsInput').value) };
...
async function saveTask(){
  ...
  if(state.editingTaskId){
    ...
    task.text = text; task.due = due; task.tags = tags;
  } else {
    state.tasks.unshift({ id:`task-${Date.now()}`, done:false, text, due, tags });
  }
  await saveTaskFile();
  renderAll();
  closeEditor();
...
function loadDemoTasks(){
  state.tasks = [
    { id:'demo-1', done:false, text:'Finish dissertation chapter outline', due:todayStr(), tags:['school'] },
    { id:'demo-2', done:false, text:'Review fatigue article highlights', due:'2026-04-10', tags:['research','school'] },
    { id:'demo-3', done:false, text:'Schedule dentist appointment', due:'2026-04-08', tags:['personal'] },
    { id:'demo-4', done:true, text:'Update Atomic Notes MD app', due:'2026-04-05', tags:['projects'] }
  ];
  renderAll();
}
